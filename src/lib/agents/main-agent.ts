...
- get_api_usage — このアプリ自身がClaude APIに使った金額・リクエスト数・トークン数を
  読み取る（今日と今月、モデル別の内訳）。「APIの使用状況」「いくら使ってる」等を
  聞かれたら推測せず必ずこれを使うこと。なお、これはこのアプリの消費だけで、
  Claude Code やコンソールなど同じキーを使う他のものは含まれない旨も必要なら伝える。
